...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.73-CALENDAR-CHALLENGE-SECURITY") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/accountbook-shell-v22873.css"; Type = "text/css" },
  ...
  Write-Host "PASS: V22.8.73 calendar, challenge, and security merge"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: confirm /health skill_caller_auth_configured, /ready reports 11 tables, then check the calendar dot, day detail edit/delete, and the three challenge types."
}
finally {
  $client.Dispose()
}
