...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.67-ROLE-SURFACE-EMPTY-STATE") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/accountbook-shell-v22865.css"; Type = "text/css" },
  ...
  Write-Host "PASS: V22.8.67 role surface and empty state"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: sign in as a 구성원 or 조회 전용 member, open /keyword-guide, and confirm the save button is gone and the 보기 전용 notice is shown."
}
finally {
  $client.Dispose()
}
