...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.44-THEME-CONTRAST-ACCESSIBILITY") { throw "/health mismatch: $($healthResult.Body)" }
  ...
  if ($shell.Status -ne 200 -or $shell.ContentType -notmatch "text/css" -or $shell.CacheControl -notmatch "immutable" -or $shell.ETag -ne '"accountbook-shell-v22844-css"') {
    throw "shell asset mismatch: status=$($shell.Status), type=$($shell.ContentType), cache=$($shell.CacheControl), etag=$($shell.ETag)"
  }
  foreach ($marker in @("--ab12-muted:#5f6b7a", "@media(prefers-contrast:more)", "@media(forced-colors:active)")) {
    if (-not $shell.Body.Contains($marker)) { throw "shell marker missing: $marker" }
  }

  $goalAsset = Get-Result "/assets/accountbook-goals-v22843.js"
  if ($goalAsset.Status -ne 200 -or $goalAsset.ContentType -notmatch "javascript" -or $goalAsset.CacheControl -notmatch "immutable") { throw "goal asset mismatch" }
  ...
  foreach ($check in @(
    ...
    @{ Path = "/cron/recurring/apply?key=v22844-no-secret"; Status = 404 },
    @{ Path = "/ops-snapshot.json?key=v22844-no-secret"; Status = 401 }
  )) {
  ...
  Write-Host "PASS: V22.8.44 theme contrast accessibility"
