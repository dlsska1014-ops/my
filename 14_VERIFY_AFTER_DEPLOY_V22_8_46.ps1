...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.46-MEMBER-ROLE-SCHEMA-ALIGNMENT") { throw "/health mismatch: $($healthResult.Body)" }
  ...
  foreach ($check in @(
    ...
    @{ Path = "/cron/recurring/apply?key=v22846-no-secret"; Status = 404 },
    @{ Path = "/ops-snapshot.json?key=v22846-no-secret"; Status = 401 }
  )) {
  ...
  Write-Host "PASS: V22.8.46 member role schema alignment"
