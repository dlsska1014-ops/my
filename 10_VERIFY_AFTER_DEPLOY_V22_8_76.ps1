...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.76-KAKAO-INTENT-SKILL-AUTH") {
  ...
  foreach ($assetCheck in @(
    ...
    @{ Path = "/assets/accountbook-shell-v22874.css"; Type = "text/css" },
  ...
  Write-Host "PASS: V22.8.76 kakao intent and skill auth"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: open /health and read skill_caller_auth. If denied is climbing while accepted stays 0, the OpenBuilder header is missing - set KAKAO_SKILL_AUTH_MODE=observe to restore service while you fix it."
}
finally {
  $client.Dispose()
}
