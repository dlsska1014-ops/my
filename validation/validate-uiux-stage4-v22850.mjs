...
ok(source.includes('const APP_VERSION = "V22.8.50-UIUX-STAGE4-QUICK-DOCK-SAVE-FEEDBACK"'), "stage 4 runtime version is explicit");
...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22850.css"'), "stage 4 uses a fresh shell asset");
ok(source.includes('const ACCOUNTBOOK_STAGE4_NAV_JS_ASSET_PATH = "/assets/accountbook-nav-v22850.js"'), "stage 4 uses a fresh navigation asset");
ok(source.includes('const ACCOUNTBOOK_V5_BUNDLE_JS_ASSET_PATH = "/assets/accountbook-v5-v22850.js"'), "stage 4 uses a fresh V5 asset");
ok(source.includes('"accountbook-shell-v22850-css"'), "stage 4 shell has a fresh immutable ETag");
ok(source.includes('"accountbook-nav-v22850-js"'), "stage 4 navigation has a fresh immutable ETag");
ok(source.includes('"accountbook-v5-v22850-js"'), "stage 4 V5 runtime has a fresh immutable ETag");
...
try {
  ...
  ok(home.text.includes('/assets/accountbook-shell-v22850.css'), "home loads the stage 4 shell");
  ok(home.text.includes('/assets/accountbook-nav-v22850.js'), "home loads the stage 4 navigation runtime");
  ok(home.text.includes('/assets/accountbook-v5-v22850.js'), "home loads the stage 4 V5 runtime");
  ...
  const shell = await request(fixture, "/assets/accountbook-shell-v22850.css");
  eq(shell.response.status, 200, "stage 4 shell is served");
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v22850-css"', "stage 4 shell ETag is correct");
  ...
  const nav = await request(fixture, "/assets/accountbook-nav-v22850.js");
  eq(nav.response.status, 200, "stage 4 navigation runtime is served");
  eq(nav.response.headers.get("etag"), '"accountbook-nav-v22850-js"', "stage 4 navigation ETag is correct");

  const v5 = await request(fixture, "/assets/accountbook-v5-v22850.js");
  eq(v5.response.status, 200, "stage 4 V5 runtime is served");
  eq(v5.response.headers.get("etag"), '"accountbook-v5-v22850-js"', "stage 4 V5 runtime ETag is correct");
