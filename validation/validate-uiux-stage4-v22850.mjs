...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v2299.css"'), "current release uses a fresh shell asset");
...
ok(source.includes('"accountbook-shell-v2299-css"'), "current shell has a fresh immutable ETag");
...
try {
  ...
  ok(home.text.includes('/assets/accountbook-shell-v2299.css'), "home loads the current shell");
  ...
  const shell = await request(fixture, "/assets/accountbook-shell-v2299.css");
  eq(shell.response.status, 200, "stage 4 shell is served");
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v2299-css"', "current shell ETag is correct");
