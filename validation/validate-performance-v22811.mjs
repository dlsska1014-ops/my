...
try {
  ...
  eq(countOf(homeHtml, 'href="/assets/accountbook-shell-v22910.css"'), 1, "home loads the current shell stylesheet once");
  ...
  const cssPaths = ["/assets/mobile-home-v2290.css", "/assets/accountbook-shell-v22811.css", "/assets/accountbook-shell-v22910.css"];
...
eq(createHash("sha256").update(legacyBytes).digest("hex"), "589feb25681ad8f38f68bf66c783f1fe2ae482c5ca03c246d05b2dd9e1c1c071", "legacy home runtime bytes remain pinned");
  ...
  const shellCssResponse = await request("/assets/accountbook-shell-v22910.css");
  ...
  eq(countOf(householdsHtml, 'href="/assets/accountbook-shell-v22910.css"'), 1, "accountbook management loads the shell once");
  ...
  ok(householdsHtml.lastIndexOf('href="/assets/accountbook-shell-v22910.css"') > householdsHtml.lastIndexOf("</style>"), "accountbook shell is the final stylesheet cascade");

  const backup = await request("/my/backup-login?return_to=%2Fapp");
  const backupHtml = await backup.text();
  eq(countOf(backupHtml, 'href="/assets/accountbook-shell-v22910.css"'), 1, "account security loads the shell once");
  ...
  eq(countOf(loginHtml, 'href="/assets/accountbook-shell-v22910.css"'), 1, "login loads the shell once");
  ...
  for (const path of userShellPaths) {
    ...
    ok(response.status === 200 && countOf(html, 'href="/assets/accountbook-shell-v22910.css"') === 1 && countOf(html, 'src="/assets/accountbook-theme-v2299.js"') === 1 && html.includes("abV22812Shell") && hasUserNavigation && hasRemainingPageContract, `${path} receives one centralized user navigation and the V5 authenticated-page contract exactly once`);
