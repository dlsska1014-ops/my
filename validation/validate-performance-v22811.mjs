...
try {
  ...
  eq(countOf(homeHtml, 'href="/assets/mobile-home-v2290.css"'), 1, "home loads the byte-preserved base stylesheet once");
  eq(countOf(homeHtml, 'href="/assets/accountbook-shell-v22911.css"'), 1, "home loads the current shell stylesheet once");
  ...
  const cssPaths = ["/assets/mobile-home-v2290.css", "/assets/accountbook-shell-v22811.css", "/assets/accountbook-shell-v22911.css"];
  for (const path of cssPaths) {
    ...
    if (path === "/assets/mobile-home-v2290.css") {
      ...
      eq(createHash("sha256").update(bytes).digest("hex"), "1de74b06538d88685c7e4ac7dbb7f30cfd944f5149e921845df572f1bb57f3a2", "legacy home stylesheet bytes remain pinned");
    }
  }
...
eq(createHash("sha256").update(legacyBytes).digest("hex"), "589feb25681ad8f38f68bf66c783f1fe2ae482c5ca03c246d05b2dd9e1c1c071", "legacy home runtime bytes remain pinned");
  ...
  const shellCssResponse = await request("/assets/accountbook-shell-v22911.css");
  ...
  eq(countOf(householdsHtml, 'href="/assets/accountbook-shell-v22911.css"'), 1, "accountbook management loads the shell once");
  ...
  ok(householdsHtml.lastIndexOf('href="/assets/accountbook-shell-v22911.css"') > householdsHtml.lastIndexOf("</style>"), "accountbook shell is the final stylesheet cascade");

  const backup = await request("/my/backup-login?return_to=%2Fapp");
  const backupHtml = await backup.text();
  eq(countOf(backupHtml, 'href="/assets/accountbook-shell-v22911.css"'), 1, "account security loads the shell once");
  ...
  eq(countOf(loginHtml, 'href="/assets/accountbook-shell-v22911.css"'), 1, "login loads the shell once");
  ...
  for (const path of userShellPaths) {
    ...
    ok(response.status === 200 && countOf(html, 'href="/assets/accountbook-shell-v22911.css"') === 1 && countOf(html, 'src="/assets/accountbook-theme-v2299.js"') === 1 && html.includes("abV22812Shell") && hasUserNavigation && hasRemainingPageContract, `${path} receives one centralized user navigation and the V5 authenticated-page contract exactly once`);
