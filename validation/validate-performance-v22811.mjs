...
try {
  ...
  const cssPaths = ["/assets/mobile-home-v22912.css", "/assets/accountbook-shell-v22811.css", "/assets/accountbook-shell-v22913.css"];
  for (const path of cssPaths) {
    ...
    if (path === "/assets/mobile-home-v22912.css") {
      ...
      eq(createHash("sha256").update(bytes).digest("hex"), "f85b82284538b3d2bb47b84de7e49236d5ac612422d320626451c72bef528b60", "legacy home stylesheet bytes remain pinned");
    }
  }

  const legacyJs = await request("/assets/mobile-home-v2298.js");
  const legacyBytes = Buffer.from(await legacyJs.arrayBuffer());
  const legacyJsGetDatabaseCalls = calls.length;
  const legacyHead = await request("/assets/mobile-home-v2298.js", { method: "HEAD" });
  const legacyJsHeadDatabaseCalls = calls.length;
  // V22.9.8: 공유로 들어온 값을 스스로 한 번 파싱하는 줄이 이 런타임에 붙었다. 바이트
// 고정 자산이므로 주소를 올린다(v2296 → v2298).
// 고정 해시도 새 주소의 것으로 함께 옮긴다.
eq(createHash("sha256").update(legacyBytes).digest("hex"), "589feb25681ad8f38f68bf66c783f1fe2ae482c5ca03c246d05b2dd9e1c1c071", "legacy home runtime bytes remain pinned");
  eq(legacyJs.headers.get("etag"), '"mobile-home-v2298-js"', "legacy home runtime ETag remains pinned");
  ...
  const shellJs = await request("/assets/mobile-home-shell-v2298.js");
  const shellRuntime = await shellJs.text();
  const shellJsGetDatabaseCalls = calls.length;
  const shellHead = await request("/assets/mobile-home-shell-v2298.js", { method: "HEAD" });
  ...
  const shellCssResponse = await request("/assets/accountbook-shell-v22913.css");
  ...
  const themeJs = await request("/assets/accountbook-theme-v2299.js");
  const themeRuntime = await themeJs.text();
  const themeJsGetDatabaseCalls = calls.length;
  const themeHead = await request("/assets/accountbook-theme-v2299.js", { method: "HEAD" });
  ...
  eq(countOf(householdsHtml, 'href="/assets/accountbook-shell-v22913.css"'), 1, "accountbook management loads the shell once");
  ...
  ok(householdsHtml.lastIndexOf('href="/assets/accountbook-shell-v22913.css"') > householdsHtml.lastIndexOf("</style>"), "accountbook shell is the final stylesheet cascade");

  const backup = await request("/my/backup-login?return_to=%2Fapp");
  const backupHtml = await backup.text();
  eq(countOf(backupHtml, 'href="/assets/accountbook-shell-v22913.css"'), 1, "account security loads the shell once");
  ...
  eq(countOf(loginHtml, 'href="/assets/accountbook-shell-v22913.css"'), 1, "login loads the shell once");
  ...
  for (const path of userShellPaths) {
    ...
    ok(response.status === 200 && countOf(html, 'href="/assets/accountbook-shell-v22913.css"') === 1 && countOf(html, 'src="/assets/accountbook-theme-v2299.js"') === 1 && html.includes("abV22812Shell") && hasUserNavigation && hasRemainingPageContract, `${path} receives one centralized user navigation and the V5 authenticated-page contract exactly once`);
  ...
  for (const item of excludedShellPaths) {
    ...
    // V22.9.9: 판 번호를 박아 두면 자산 주소를 올릴 때마다 이 보장이 조용히 헐거워진다
    // (v22879 만 막고 v2299 는 통과한다). 셸 자산처럼 판 번호를 가리지 않고 본다.
    const hasAnyUserThemeAsset = /accountbook-theme-v\d+/i.test(html);
    ok(response.status === 200 && hasExpectedScope && !html.includes('data-nav-scope="user"') && !hasAnyUserShellAsset && !hasAnyUserThemeAsset && !html.includes("abV22812Shell"), `${item.path} stays outside every version of the user theme shell${item.scope ? ` with ${item.scope} scope` : ""} (status=${response.status}, expectedScope=${hasExpectedScope}, userScope=${html.includes('data-nav-scope="user"')}, shellAsset=${hasAnyUserShellAsset}, themeAsset=${hasAnyUserThemeAsset}, shellClass=${html.includes("abV22812Shell")})`);
  }
