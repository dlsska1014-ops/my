...
try {
  ...
  const legacyJs = await request("/assets/mobile-home-v2290.js");
  const legacyBytes = Buffer.from(await legacyJs.arrayBuffer());
  const legacyJsGetDatabaseCalls = calls.length;
  const legacyHead = await request("/assets/mobile-home-v2290.js", { method: "HEAD" });
...
eq(createHash("sha256").update(legacyBytes).digest("hex"), "c78fab08d1cad3ed3bf6977304a07f4a0c39d59992bbca21e45a517c409c6062", "legacy home runtime bytes remain pinned");
  eq(legacyJs.headers.get("etag"), '"mobile-home-v2290-js"', "legacy home runtime ETag remains pinned");
  ...
  const shellJs = await request("/assets/mobile-home-shell-v22879.js");
  const shellRuntime = await shellJs.text();
  const shellJsGetDatabaseCalls = calls.length;
  const shellHead = await request("/assets/mobile-home-shell-v22879.js", { method: "HEAD" });
