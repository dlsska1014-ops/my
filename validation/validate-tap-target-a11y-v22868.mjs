...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22911.css"'), "shell asset moved to a fresh immutable path");
ok(source.includes('"accountbook-shell-v22911-css"'), "shell asset has a fresh immutable ETag");
...
try {
  ...
  for (const [name, path] of PAGES) {
    ...
    if (page.text.includes("abV22812Shell")) {
      eq((page.text.match(/href="\/assets\/accountbook-shell-v22911\.css"/g) || []).length, 1, `${name}: 새 셸 스타일시트를 한 번만 부른다`);
    }
  }
