...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22899.css"'), "shell stylesheet moved to a new immutable address");
ok(source.includes('\'"accountbook-shell-v22899-css"\''), "shell stylesheet ETag matches its new path");
...
let shellCss = "";
try {
  const response = await app.fetch(new Request("https://ttokttok-accountbook.com/assets/accountbook-shell-v22899.css"), fixture.env, {});
  eq(response.status, 200, "the bumped shell stylesheet is served");
  eq(response.headers.get("etag"), '"accountbook-shell-v22899-css"', "the served ETag matches the path");
...
const consumedWithFallback = [
  ...
  ["--ab12-sp-3", "12px"],
  // V22.8.91(4.3): 리포트 4장이 간격·타이포·고도 토큰을 읽는다.
  ["--ab12-sp-4", "16px"],
  ["--ab12-sp-2", "8px"],
  ["--ab12-fs-title", "16px"],
  ["--ab12-fs-num-lg", "22px"],
  ["--ab12-fw-num-lg", "700"],
  ["--ab12-elev-card", "0 1px 2px rgba(0,0,0,.04)"],
];
...
try {
  ...
  eq((html.match(/href="\/assets\/accountbook-shell-v22899\.css"/g) || []).length, 1, "home loads the bumped shell exactly once");
