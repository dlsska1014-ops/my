...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22891.css"'), "shell stylesheet moved to a new immutable address");
ok(source.includes('\'"accountbook-shell-v22891-css"\''), "shell stylesheet ETag matches its new path");
...
let shellCss = "";
try {
  const response = await app.fetch(new Request("https://ttokttok-accountbook.com/assets/accountbook-shell-v22891.css"), fixture.env, {});
  eq(response.status, 200, "the bumped shell stylesheet is served");
  eq(response.headers.get("etag"), '"accountbook-shell-v22891-css"', "the served ETag matches the path");
...
try {
  ...
  eq((html.match(/href="\/assets\/accountbook-shell-v22891\.css"/g) || []).length, 1, "home loads the bumped shell exactly once");
