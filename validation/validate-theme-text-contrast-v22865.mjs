...
ok(source.includes('const APP_VERSION = "V22.8.73-CALENDAR-CHALLENGE-SECURITY"'), "runtime exposes the contrast release");
...
try {
  ...
  for (const [name, path] of [["연간 리포트", "/annual?year=2026&household_id=house-home"], ["자산·계좌", `/payment-methods?${q}`], ["종합 리포트", `/my/analysis?${q}&view=report`], ["스마트 도구", `/smart-tools?${q}`]]) {
    ...
    eq(page.text.split('href="/assets/accountbook-shell-v22873.css"').length - 1, 1, `${name} 화면이 새 셸을 정확히 한 번 참조한다`);
