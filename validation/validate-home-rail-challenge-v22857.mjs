...
ok(source.includes('const APP_VERSION = "V22.8.67-ROLE-SURFACE-EMPTY-STATE"'), "current runtime version is explicit");
...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22865.css"'), "changed shell uses a fresh immutable asset URL");
...
ok(source.includes('"accountbook-shell-v22865-css"'), "shell ETag is refreshed");
...
try {
  ...
  ok(home.text.includes('/assets/accountbook-shell-v22865.css'), "home loads the refreshed shell asset");
  ...
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v22865-css"', "refreshed shell ETag is correct");
