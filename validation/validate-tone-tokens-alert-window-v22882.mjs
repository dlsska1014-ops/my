...
ok(source.includes('const APP_VERSION = "V22.8.100-DEAD-MARKUP"'), "runtime exposes the V22.8.82 release");
...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22899.css"'), "the shell stylesheet path is bumped");
ok(source.includes('\'"accountbook-shell-v22899-css"\''), "the shell stylesheet ETag matches its path");
...
const shellRes = await get("/assets/accountbook-shell-v22899.css");
eq(shellRes.status, 200, "the bumped shell stylesheet is served");
eq(shellRes.headers.get("etag"), '"accountbook-shell-v22899-css"', "the served ETag matches the path");
