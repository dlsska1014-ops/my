...
ok(source.includes('const APP_VERSION = "V22.8.60-FUNCTIONAL-UI-RELIABILITY"'), "runtime exposes the functional UI reliability release");
...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22860.css"'), "changed shell uses a new immutable path");
ok(source.includes('const ACCOUNTBOOK_STAGE4_NAV_JS_ASSET_PATH = "/assets/accountbook-nav-v22860.js"'), "changed navigation uses a new immutable path");
ok(source.includes('const ACCOUNTBOOK_V5_BUNDLE_JS_ASSET_PATH = "/assets/accountbook-v5-v22860.js"'), "changed challenge runtime uses a new immutable path");
...
try {
  ...
  const shell = await request(fixture, "/assets/accountbook-shell-v22860.css", { cookie: "" });
  eq(shell.response.status, 200, "new shell asset is served");
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v22860-css"', "new shell ETag is correct");
  ...
  const nav = await request(fixture, "/assets/accountbook-nav-v22860.js", { cookie: "" });
  eq(nav.response.status, 200, "new navigation asset is served");
  eq(nav.response.headers.get("etag"), '"accountbook-nav-v22860-js"', "new navigation ETag is correct");
  ...
  const v5 = await request(fixture, "/assets/accountbook-v5-v22860.js", { cookie: "" });
  eq(v5.response.status, 200, "new V5 runtime asset is served");
  eq(v5.response.headers.get("etag"), '"accountbook-v5-v22860-js"', "new V5 runtime ETag is correct");
