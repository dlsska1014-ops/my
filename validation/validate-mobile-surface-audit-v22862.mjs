...
ok(source.includes('const APP_VERSION = "V22.9.0-UX-REPAIR"'), "runtime exposes the audit fix release");
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22912.css"'), "changed shell uses a new immutable path");
...
try {
  ...
  const shell = await request(fixture, "/assets/accountbook-shell-v22912.css", { cookie: "" });
  eq(shell.response.status, 200, "new shell asset is served");
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v22912-css"', "new shell ETag is correct");
  ...
  for (const [name, path] of tabs) {
    ...
    const shellRefs = page.text.split('href="/assets/accountbook-shell-v22912.css"').length - 1;
