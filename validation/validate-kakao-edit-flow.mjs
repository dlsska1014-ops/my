...
try {
  ...
  // 복구 버퍼 저장 실패 시 삭제까지 진행하면 되돌릴 수 없는 데이터 손실이 된다.
  fixture.db.__fail_next_settings_write = true;
  const blockedDelete = await say("삭제 02번");
  ok(!blockedDelete.includes("삭제했어요"), "[E2E·안전] 복구 버퍼 저장 실패를 삭제 성공으로 안내하지 않음");
  ok(txById("tx-edit-2"), "[E2E·안전] 복구 버퍼 저장 실패 시 원본 거래를 삭제하지 않음");
  ...
  const retryRestore = await say("복구 02번");
  ok(retryRestore.includes("복구했어요") && txById("tx-edit-2"), "[E2E·안전] 보존된 버퍼로 재시도 성공");
