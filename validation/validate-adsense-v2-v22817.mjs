...
try {
  ...
  const shellResponse = await request("/assets/accountbook-shell-v22846.css");
  eq(shellResponse.status, 200, "V22.8.38 V5 stabilization stylesheet is served");
  eq(shellResponse.headers.get("etag"), '"accountbook-shell-v22846-css"', "V22.8.46 contrast-safe shell has a new immutable ETag");
  ...
  eq(countOf(calendarHomeHtml, 'href="/assets/accountbook-shell-v22846.css"'), 1, "home loads the V22.8.46 shell exactly once");
  ...
  ok(insightHtml.includes('/my/analysis/app.js?v=V22.8.46-MEMBER-ROLE-SCHEMA-ALIGNMENT'), "analysis keeps the protected external runtime with the new cache version");
  eq(countOf(insightHtml, 'href="/assets/accountbook-shell-v22846.css"'), 1, "analysis loads the V22.8.46 shell exactly once");
