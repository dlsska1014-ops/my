...
try {
  ...
  const shellResponse = await request("/assets/accountbook-shell-v22913.css");
  eq(shellResponse.status, 200, "V22.8.38 V5 stabilization stylesheet is served");
  eq(shellResponse.headers.get("etag"), '"accountbook-shell-v22913-css"', "current shell has a fresh immutable ETag");
  ...
  eq(countOf(calendarHomeHtml, 'href="/assets/accountbook-shell-v22913.css"'), 1, "home loads the current shell exactly once");
  ...
  eq(countOf(insightHtml, 'href="/assets/accountbook-shell-v22913.css"'), 1, "analysis loads the current shell exactly once");
