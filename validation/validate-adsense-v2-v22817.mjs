...
try {
  ...
  const shellResponse = await request("/assets/accountbook-shell-v22859.css");
  eq(shellResponse.status, 200, "V22.8.38 V5 stabilization stylesheet is served");
  eq(shellResponse.headers.get("etag"), '"accountbook-shell-v22859-css"', "current shell has a fresh immutable ETag");
  ...
  ok(source.includes('const ACCOUNTBOOK_STAGE4_NAV_JS_ASSET_PATH = "/assets/accountbook-nav-v22850.js"'), "V5 navigation ships as a separately versioned asset");

  const stage4NavResponse = await request("/assets/accountbook-nav-v22850.js");
  eq(stage4NavResponse.status, 200, "V5 navigation runtime is served");
  eq(stage4NavResponse.headers.get("etag"), '"accountbook-nav-v22850-js"', "V5 navigation runtime has the reviewed immutable ETag");
  ...
  eq(countOf(calendarHomeHtml, 'href="/assets/accountbook-shell-v22859.css"'), 1, "home loads the current shell exactly once");
  eq(countOf(calendarHomeHtml, 'src="/assets/accountbook-nav-v22850.js"'), 1, "home loads the V5 navigation runtime exactly once after preserved runtimes");
  ...
  ok(insightHtml.includes('/my/analysis/app.js?v=V22.8.59-ACCOUNT-RUNTIME-RELIABILITY'), "analysis keeps the protected external runtime with the new cache version");
  eq(countOf(insightHtml, 'href="/assets/accountbook-shell-v22859.css"'), 1, "analysis loads the current shell exactly once");
