...
ok(source.includes('const APP_VERSION = "V22.8.88-HOME-BLOCKS-DAILY"'), "runtime exposes the budget and dark contrast release");
...
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22888.css"'), "the shell stylesheet moved to a new immutable address");
ok(source.includes('"accountbook-shell-v22888-css"'), "the shell stylesheet ships a matching ETag");

const ORIGIN = "https://ttokttok-accountbook.com";
const fixture = await createV2265QaFixture();
try {
  // 규칙이 실제로 셸 자원에 실려 나가는지 확인한다. 소스에만 있고 서빙되지 않으면 소용없다.
  const shell = await app.fetch(new Request(`${ORIGIN}/assets/accountbook-shell-v22888.css`), fixture.env, {});
