...
ok(source.includes('const APP_VERSION = "V22.8.62-MOBILE-SURFACE-AUDIT-FIX"'), "runtime exposes the mobile surface repair release");
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22862.css"'), "changed shell uses a new immutable path");
...
try {
  ...
  const shell = await request(fixture, "/assets/accountbook-shell-v22862.css", { cookie: "" });
  eq(shell.response.status, 200, "new shell asset is served");
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v22862-css"', "new shell ETag is correct");
  ...
  ok(home.text.includes("/assets/accountbook-shell-v22862.css"), "home loads the refreshed shell");
