...
ok(source.includes('const APP_VERSION = "V22.8.61-MOBILE-SURFACE-REPAIR"'), "runtime exposes the mobile surface repair release");
ok(source.includes('const ACCOUNTBOOK_SHELL_CSS_ASSET_PATH = "/assets/accountbook-shell-v22861.css"'), "changed shell uses a new immutable path");
ok(source.includes('const ACCOUNTBOOK_STAGE4_NAV_JS_ASSET_PATH = "/assets/accountbook-nav-v22861.js"'), "changed navigation uses a new immutable path");
...
try {
  ...
  const shell = await request(fixture, "/assets/accountbook-shell-v22861.css", { cookie: "" });
  eq(shell.response.status, 200, "new shell asset is served");
  eq(shell.response.headers.get("etag"), '"accountbook-shell-v22861-css"', "new shell ETag is correct");
  ...
  const nav = await request(fixture, "/assets/accountbook-nav-v22861.js", { cookie: "" });
  eq(nav.response.status, 200, "new navigation asset is served");
  eq(nav.response.headers.get("etag"), '"accountbook-nav-v22861-js"', "new navigation ETag is correct");
  ...
  ok(home.text.includes("/assets/accountbook-nav-v22861.js"), "home loads the refreshed navigation runtime");
