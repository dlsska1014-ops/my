...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.79-UIUX-INCOME-BUDGET-THEME") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/mobile-home-v22879.css"; Type = "text/css" },
    @{ Path = "/assets/accountbook-shell-v22879.css"; Type = "text/css" },
    @{ Path = "/assets/accountbook-theme-v22879.js"; Type = "javascript" },
    @{ Path = "/assets/mobile-home-v22879.js"; Type = "javascript" },
    @{ Path = "/assets/mobile-home-shell-v22879.js"; Type = "javascript" },
    @{ Path = "/assets/accountbook-nav-v22879.js"; Type = "javascript" }
  )) {
  ...
  foreach ($retiredAsset in @(
    "/assets/accountbook-shell-v22874.css",
    "/assets/mobile-home-v22875.css",
    "/assets/accountbook-theme-v22812.js",
    "/assets/mobile-home-v22870.js",
    "/assets/accountbook-nav-v22862.js"
  )) {
    $retired = Get-Result $retiredAsset
    if ($retired.Status -ne 404) { throw "$retiredAsset should be retired but returned HTTP $($retired.Status)" }
  }
  ...
  Write-Host "PASS: V22.8.79 UI/UX income budget theme"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host ""
  Write-Host "Next (eyes on a real device):"
  Write-Host " 1. /app with no parameters shows this month; /app?month=2026-07 offers the go-back link"
  Write-Host " 2. Sidebar logo is crisp in both light and dark"
  Write-Host " 3. Appearance settings offer light and dark only"
  Write-Host " 4. /app?tab=transactions shows the filter, search and category dropdown"
  Write-Host " 5. /reserve-plans can edit an item, mark it income, and mark it monthly"
  Write-Host " 6. Home shows the income usage gauge and the day/week/month period tabs"
}
finally {
  $client.Dispose()
}
