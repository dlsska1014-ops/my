...
assert((await health.text()).includes('V22.6.9-SECURITY-SPENDER-PRIVACY-HOTFIX'), 'health version V22.4');
...
assert(r.text.includes('가계부 이름') && r.text.includes('이 이름으로 만들까요'), 'name-like kind input enters confirmation');
...
assert(r.text.includes('이 이름으로 만들까요'), 'standard type-name flow enters confirmation');
r = await skill('응', 'direct-user', '다른사용자');
assert(r.text.includes('가계부를 만들었어요'), 'standard type-name flow still works after confirmation');
