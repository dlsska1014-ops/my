...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.71-EDIT-RESTORE-FIX") {
  ...
  Write-Host "PASS: V22.8.71 edit and restore round trip"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: in KakaoTalk send 수정 01번 금액 3만5천원, then delete two records and restore both by the numbers you were told."
}
finally {
  $client.Dispose()
}
