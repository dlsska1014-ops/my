...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.74-BUDGET-DARK-CONTRAST") {
  ...
  Write-Host "PASS: V22.8.74 budget review and dark contrast"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: switch to dark mode and confirm the empty-state boxes and the skip-to-content badge are readable, then press Tab once on any screen."
}
finally {
  $client.Dispose()
}
