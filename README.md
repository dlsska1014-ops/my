# 똑똑한 가계부 V22.8.12 접근성 테마·대비

V22.8.11의 홈 UX 셸과 모든 보안·카카오·영수증·분석 보호 기준을 유지하면서, 실제 운영 화면에서 확인된 저대비 텍스트를 수정하고 시스템·라이트·다크 화면 모드와 네 가지 컬러톤을 추가한 누적 배포본입니다.
...
- 대비 수정: `SMART NOTICE`, 분석 보조 텍스트, 모바일 메뉴, placeholder, 요약·정산 안내
...
| 경로 | 역할 |
|---|---|
| `/assets/accountbook-shell-v22812.css` | 접근성 대비와 라이트·다크·컬러톤 셸 |
...
- UX·분석 보호 55개
- 접근성 테마·홈 셸·성능 112개
- 합계 283개와 ESM `default.fetch`

## 적용 후 확인

1. `/health` 버전이 `V22.8.12-ACCESSIBLE-THEME-CONTRAST`인지 확인합니다.
2. 전체 메뉴의 화면 설정에서 시스템·라이트·다크와 네 가지 컬러톤을 확인합니다.
3. 새로고침과 같은 브라우저의 다른 화면에서 선택값이 유지되는지 확인합니다.
4. PC Chrome, iPhone Safari, Android Chrome에서 홈·분석·설정·하단 메뉴 대비를 확인합니다.
5. 새 CSS·JavaScript와 보존된 이전 자원 경로가 모두 200인지 확인합니다.
6. 기록 저장, 가계부 전환, 계정 보안, 분석, 영수증 OCR, 카카오 1:1·그룹을 확인합니다.

## 롤백

DB 변경이 없으므로 문제가 생기면 직전 V22.8.11 `src/index.js`로 Worker 코드만 전체 롤백합니다. SQL·환경변수·외부 콘솔 롤백은 없습니다.
