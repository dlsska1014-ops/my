# 똑똑한 가계부 V22.8.20 통합 안전본

V22.8.19의 보안·영수증·분석·테마·AdSense 심사·전체 대비 기준을 유지하면서, Claude 브랜치의 카카오 수정 V4와 웹 인라인 피드백을 선별 반영한 누적 배포본입니다.
...
| 경로 | 역할 |
|---|---|
| `/assets/accountbook-shell-v22819.css` | V2·UI/UX 4단계와 전체 대비 보정 최종 셸 |
...
- AdSense 심사·V2·UI·UX 4단계 화면 261개
- 합계 705개와 ESM `default.fetch`

## 적용 후 확인

1. `/health`가 `V22.8.20-KAKAO-EDIT-INLINE-FEEDBACK`인지 확인합니다.
...
5. `/assets/accountbook-shell-v22819.css`의 200·CSS MIME·immutable·ETag를 확인합니다.
6. PC와 모바일에서 4단계 메뉴·홈·달력·분석·다크모드·네 컬러톤과 가로 넘침을 확인합니다.
7. 기록 저장, 가계부 전환, 계정 보안, 영수증 OCR, 카카오 1:1·그룹의 조회·수정·삭제·복구를 확인합니다.

## 롤백

DB와 외부 콘솔 변경이 없으므로 문제가 생기면 직전 V22.8.19 `src/index.js`로 Worker 코드만 전체 롤백합니다.
