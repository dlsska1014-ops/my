# 똑똑한 가계부 V22.8.14 홈 전체 조회 버튼 대비 핫픽스

V22.8.13의 다크모드 전체 보정과 모든 보안·카카오·영수증·분석 보호 기준을 유지하면서, 실제 운영 데이터 11건에서만 나타난 홈 `전체 N건 조회` 링크 버튼의 대비를 교정한 핫픽스입니다.
...
- 대비 수정: 홈 기록 10건 초과 시 표시되는 전체 조회 링크 버튼
...
| 경로 | 역할 |
|---|---|
| `/assets/accountbook-shell-v22814.css` | 홈 전체 조회 버튼 대비를 포함한 최신 셸 |
...
- 합계 291개와 ESM `default.fetch`

## 적용 후 확인

1. `/health` 버전이 `V22.8.14-HOME-FEED-BUTTON-CONTRAST`인지 확인합니다.
...
DB 변경이 없으므로 문제가 생기면 직전 V22.8.13 `src/index.js`로 Worker 코드만 전체 롤백합니다. SQL·환경변수·외부 콘솔 롤백은 없습니다.
