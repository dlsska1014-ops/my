# 똑똑한 가계부 V22.8.13 다크모드 전체 보정

V22.8.12의 화면 모드와 컬러톤, 모든 보안·카카오·영수증·분석 보호 기준을 유지하면서 운영 점검에서 확인된 예산·정산·내 설정의 다크모드 누락과 링크 버튼·분석 필터 대비를 보정한 누적 배포본입니다.
...
- 대비 수정: 예산·고급 정산·내 설정·키워드 편집기·링크 버튼·분석 비활성 필터
...
| 경로 | 역할 |
|---|---|
| `/assets/accountbook-shell-v22813.css` | 다크모드 전체 화면 보정과 라이트 필터 대비 셸 |
...
- 합계 289개와 ESM `default.fetch`

## 적용 후 확인

1. `/health` 버전이 `V22.8.13-DARK-MODE-FULL-COVERAGE`인지 확인합니다.
2. 전체 메뉴의 화면 설정에서 시스템·라이트·다크와 네 가지 컬러톤을 확인합니다.
3. 새로고침과 같은 브라우저의 다른 화면에서 선택값이 유지되는지 확인합니다.
4. PC Chrome, iPhone Safari, Android Chrome에서 홈·분석·예산·정산·내 설정 대비를 확인합니다.
5. 새 CSS·JavaScript와 보존된 이전 자원 경로가 모두 200인지 확인합니다.
6. 기록 저장, 가계부 전환, 계정 보안, 분석, 영수증 OCR, 카카오 1:1·그룹을 확인합니다.

## 롤백

DB 변경이 없으므로 문제가 생기면 직전 V22.8.12 `src/index.js`로 Worker 코드만 전체 롤백합니다. SQL·환경변수·외부 콘솔 롤백은 없습니다.
