...
- 처음 시작 온보딩을 **가계부 만들기 → 참여 → 기록** 순서로 재구성했습니다.
  - 가계부가 없는 사용자가 기록·조회를 시도하면 "점심 12000원" 예시 대신 1단계 가계부 만들기부터 단계적으로 안내합니다. (카카오톡 자동 가계부 생성 중단)
  - 도움말·메뉴·대표 명령어 드롭다운을 같은 순서로 재배치했습니다. (1 가계부 만들기 → 2 단톡방 연결 → 3 기록 → …)
- 로드맵 P0 정렬 (KAKAO_CHATBOT_IMPROVEMENT_ROADMAP_V1.md):
  - P0-1: 비 JSON POST는 저장 로직에 진입하지 않고 안전 응답만 반환. 원문 테스트는 `ALLOW_RAW_SKILL_TEST=1` 개발 설정 전용
  - P0-2: quickReplies 상태 라벨을 `disabled_by_baseline`으로 통일
  - P0-3: 사용자 노출 영역에서 `오늘예산` 제거, 정식 용어 `남은예산`으로 통일 (파서 호환 별칭만 유지)
  - P0-4: `가계부 만들기`·`가계부 참여`·`가계부 전환`은 DB 변경 없이 대표 주소 1개(`/my/households`)만 안내. `단톡방 연결 + 초대코드`는 기존 유지
  - P0-6/P0-9: OpenBuilder 블록을 12업무 블록으로 재작성 — 단일 원본 `KAKAO_OPENBUILDER_BLOCK_MAP_V21_4_4.xlsx` (발화 111개, 한 행 한 발화, 전 발화 파서 대응 명시, 중복 0)
  - P0-8: APP_VERSION·VERSION.txt·보고서·XLSX 버전 문자열 통일
- 검증: `node --check` + `node smoke_v2144.mjs` (30건: 온보딩/P0/회귀)
