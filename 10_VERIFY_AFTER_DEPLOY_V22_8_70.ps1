...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.70-AMOUNT-PARSE-FIX") {
  ...
  Write-Host "PASS: V22.8.70 amount parsing"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: in KakaoTalk send 커피 12,345원 and 커피 3만5천원, then confirm each is saved once with the exact amount."
}
finally {
  $client.Dispose()
}
