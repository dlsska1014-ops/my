...
assert((await health.text()).includes('V21.5.2-NATURAL-INTENT-SMART-CTA-HOTFIX'), 'health version V21.5.2');
...
r = await skill('도움말');
assert(r.text.includes('닉네임수정'), 'help includes nickname natural command');
...
assert(r.text.includes('내 가계부 목록'), 'natural household switch synonym supported');
r = await skill('단톡 연결');
