...
assert((await health.text()).includes('V22.6.9-SECURITY-SPENDER-PRIVACY-HOTFIX'), 'health version V21.5.2');
...
assert(r.text.includes('어떤 용도로 쓸 가계부'), 'create flow asks type');
...
assert(r.text.includes('이 이름으로 만들까요'), 'household name requires final confirmation');
r = await skill('응');
assert(r.text.includes('가계부를 만들었어요') && r.text.includes('초대코드'), 'household created with invite code after confirmation');
...
assert(r.text.includes('가계부를 선택했어요') || r.text.includes('어느 가계부'), 'natural household switch synonym supported');
r = await skill('단톡 연결');
