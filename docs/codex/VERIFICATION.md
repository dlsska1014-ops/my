...
회사 PC와 집 PC의 Codex CLI·IDE에서 같은 저장소 검증을 실행하기 위한 기준입니다. 현재 저장소 하네스는 V22.8.46 배포 묶음을 확인합니다.
...
1. `BUNDLE_FILE_CHECKSUMS_V22_8_46.sha256`의 배포 파일
...
7. 참여자 역할 스키마 20개
8. UX·분석 보호 56개
9. 사용자 화면·홈 버튼·테마·성능 144개
10. AdSense 심사·V2·UI V5 공통 셸 261개
11. UI V5 권한·범위·저장 안정화 41개
12. 핵심 쓰기·권한 스모크 97개
13. ESM import와 `default.fetch`
14. 작업 트리·스테이징 영역의 공백 오류

성공 결과는 총 902개 자동 검사와 현재 `src/index.js` SHA-256을 표시합니다.
...
하네스 성공은 운영 배포 승인이 아닙니다. V22.8.46 SQL 결과, Cloudflare 운영 배포, `/health`·`/ready`, 실제 참여자 권한 저장은 `RELEASE-CHECKLIST.md`에서 별도 확인합니다. 기존 V22.6.8·V22.7.0·V22.7.1 SQL은 다시 실행하지 않습니다.
