...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.77-INTERRUPTED-SAVE-RECOVERY") {
  ...
  Write-Host "PASS: V22.8.77 interrupted save recovery"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: sign out in one tab, then try saving in another - you should land on the login page, and after signing back in return to the same screen with what you typed still there."
}
finally {
  $client.Dispose()
}
