...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.64-WEB-MANIFEST") {
  ...
  Write-Host "PASS: V22.8.64 web manifest"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: confirm the manifest is served, that Add to Home Screen shows the brand icon, and that the installed app opens at /app."
}
finally {
  $client.Dispose()
}
