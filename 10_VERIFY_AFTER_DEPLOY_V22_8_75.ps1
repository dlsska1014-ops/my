...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.75-ANALYSIS-ROLE-SPLIT") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/mobile-home-v22875.css"; Type = "text/css" },
  ...
  Write-Host "PASS: V22.8.75 analysis role split"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: open 소비 분석 and 종합 리포트 and confirm 한눈에 보기 appears only on 소비 분석, and that the 종합 리포트 hero shows one blue button with two outlined ones."
}
finally {
  $client.Dispose()
}
