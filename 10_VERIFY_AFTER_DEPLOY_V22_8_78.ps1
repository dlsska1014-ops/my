...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.78-DRAFT-PERSISTENCE") {
  ...
  Write-Host "PASS: V22.8.78 draft persistence"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: start typing an entry, refresh the page, and confirm what you typed is still there."
}
finally {
  $client.Dispose()
}
