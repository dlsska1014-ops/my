...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.59-ACCOUNT-RUNTIME-RELIABILITY") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/accountbook-shell-v22859.css"; Type = "text/css" },
    @{ Path = "/assets/accountbook-nav-v22850.js"; Type = "javascript" },
    @{ Path = "/assets/accountbook-v5-v22859.js"; Type = "javascript" }
  )) {
  ...
  Write-Host "PASS: V22.8.59 account and runtime reliability"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: manually create one account, sign out, and sign in again with the same credentials."
}
finally {
  $client.Dispose()
}
