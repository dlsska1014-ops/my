...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.66-DELETE-BUTTON-FORM-FIX") {
  ...
  Write-Host "PASS: V22.8.66 transaction delete"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: on a real device open a transaction, tap 삭제, confirm, and check the record disappears."
}
finally {
  $client.Dispose()
}
