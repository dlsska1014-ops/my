...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.63-APP-ICON-ASSETS") {
  ...
  Write-Host "PASS: V22.8.63 app icon assets"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: confirm /favicon.ico and /apple-touch-icon.png return the brand icon and that no page logs a favicon 404."
}
finally {
  $client.Dispose()
}
