...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.72-SKIP-TO-CONTENT") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/accountbook-shell-v22872.css"; Type = "text/css" },
  ...
  Write-Host "PASS: V22.8.72 skip to content"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: open any screen, press Tab once, and confirm the skip-to-content link appears and jumps into the main content."
}
finally {
  $client.Dispose()
}
