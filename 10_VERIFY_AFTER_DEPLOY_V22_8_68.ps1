...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.68-TAP-TARGET-ACCESSIBILITY") {
  ...
  Write-Host "PASS: V22.8.68 tap target accessibility"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: on a phone, tap the 접기 headers (예: 종합 리포트의 도넛 차트, 자산·계좌의 상세 정보 수정) and the card-header links (최근 기록 확인, 월간 리포트) and confirm they respond on the first tap."
}
finally {
  $client.Dispose()
}
