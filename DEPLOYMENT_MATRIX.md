# 배포 적용 매트릭스

| 버전 | `src/index.js` 전체 교체 | 신규 SQL | 환경변수 | Kakao Developers | OpenBuilder | 핵심 |
|---|---:|---:|---:|---:|---:|---|
| V22.8.18 | 필요 | 없음 | 없음 | 없음 | 없음 | 웹앱 제출 위치 인라인 결과·응답 규약 |
| V22.8.17 | 필요 | 없음 | 없음 | 없음 | 없음 | 카카오 복구(undo) 실패 핫픽스 |
| V22.8.16 | 필요 | 선택 | 없음 | 없음 | 없음 | 카카오 수정 플로우 V4 전면 교체 |
| V22.8.15 | 필요 | 없음 | 없음 | 없음 | 없음 | 다크 로그인·계정보안 표면 대비 |
| V22.8.14 | 필요 | 없음 | 없음 | 없음 | 없음 | 홈 전체 조회 버튼 다크모드 대비 |
| V22.8.13 | 필요 | 없음 | 없음 | 없음 | 없음 | 예산·정산·내 설정 다크모드 전체 보정 |
| V22.8.12 | 필요 | 없음 | 없음 | 없음 | 없음 | 접근성 대비·라이트/다크/시스템·컬러톤 |
| V22.8.11 | 필요 | 없음 | 없음 | 없음 | 없음 | 홈 UX 셸 승격·주요 사용자 화면 확장 |
| V22.8.10 | 필요 | 없음 | 없음 | 없음 | 없음 | 홈 묶음 조회·공통 자원 캐시 |
| V22.8.9 | 필요 | 없음 | 없음 | 없음 | 없음 | 계정 보안·가계부 분리 |
| V22.8.8 | 필요 | 없음 | 없음 | 없음 | 없음 | 행동별 진행·비밀번호 피드백 |
| V22.8.7 | 필요 | 없음 | 없음 | 없음 | 없음 | 그룹 응답 규격 정합성 |
| V22.8.6 | 필요 | 없음 | 없음 | 없음 | 없음 | 영수증 OCR 안정화 |
| V22.8.5 | 필요 | 없음 | 없음 | 없음 | 없음 | 모바일 접근·메뉴 |
| V22.8.4 | 필요 | 없음 | 없음 | 없음 | 없음 | UI·성능 재검증 |
| V22.8.3 | 필요 | 없음 | 없음 | 없음 | 없음 | 안정화본 병합 |
| V22.8.2 | 필요 | 없음 | 없음 | 없음 | 없음 | 인증·세션 안정화 |
| V22.8.1 | 필요 | 없음 | 없음 | 없음 | 없음 | UI·UX 위계 |
| V22.8.0 | 필요 | 있음 | 당시 적용 | 당시 적용 | 당시 적용 | 자산·결제수단 기반 |

## V22.8.18 적용 판단

### Cloudflare Worker

- 적용 파일: `src/index.js`
- 방식: 검증된 파일 전체 교체
- 부분 붙여넣기: 금지
- 정적 자원 경로: 변경 없음 (V22.8.15 이하의 자원 전부 보존, 새 자산 파일 없음)
- 클라이언트 개선은 HTML `attachUiUxRuntime` 단계에서 인라인 주입되며 별도 자산 경로를 추가하지 않음
- 데이터베이스·환경·외부 콘솔·보호 화면: V22.8.17과 동일하게 변경 없음
- 문제가 생기면 V22.8.17 Worker 소스로 코드만 롤백(복구 기능 유지)

## V22.8.17 적용 판단

### Cloudflare Worker

- 적용 파일: `src/index.js`
- 방식: 검증된 파일 전체 교체
- 부분 붙여넣기: 금지
- 정적 자원 경로: 변경 없음 (V22.8.15 이하의 자원 전부 보존)
  - `/assets/accountbook-shell-v22815.css`
  - `/assets/accountbook-theme-v22812.js`
  - `/assets/accountbook-shell-v22811.css`
  - `/assets/mobile-home-shell-v22811.js`
  - `/assets/mobile-home-v22810.css`
  - `/assets/mobile-home-v22810.js`

### 데이터베이스

- 필수 SQL: 없음 — 수정 세션은 기존 `accountbook_settings` 키-값 방식을 사용
- 선택 SQL: 미인식 입력 텔레메트리 테이블 `unrecognized_inputs` (`SQL_HISTORY.md`의 V22.8.16 항목)
  - 실행하지 않아도 수정 플로우는 정상 동작하며 텔레메트리 적재만 조용히 생략됨
- 기존 스키마·RLS·RPC·인덱스: 변경 없음
- 인증·권한·데이터 조회 및 저장 로직: 변경 없음

### 환경과 외부 콘솔

- 신규 환경변수·Secret: 없음
- Kakao Developers 앱 키·Redirect URI·동의항목: 변경 없음
- OpenBuilder Skill URL·블록·엔티티·파라미터: 변경 없음

### 보호 화면

- 분석 클라이언트 SHA-256: `b73386dfddd66aa42000b7b34b6c03b7deeda3ac5824b75468db3aa269087a5d`
- 분석 렌더러 SHA-256: `0d03ce468e0203e1eb7bc344e0fc24876f74c92de492389246f5dd80ea0de58d`
- 영수증 OCR DOM·스크립트·POST 필드, 백업 적용, 로그인·계정 보안 form 유지

운영 배포와 실기기 확인은 이 저장소 작업에 포함되지 않습니다. 문제가 생기면 V22.8.16 Worker 소스로 코드만 롤백합니다. 단 V22.8.16에는 복구(undo) INSERT 실패 버그가 있으므로 복구 기능이 필요하면 V22.8.17 이상을 유지해야 합니다.
