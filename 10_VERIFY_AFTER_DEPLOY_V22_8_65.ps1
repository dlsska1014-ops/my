...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.65-THEME-TEXT-CONTRAST") {
  ...
  Write-Host "PASS: V22.8.65 theme text contrast"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: on a real device switch light/dark and all four accent tones, then read the annual report, assets hero and challenge settings."
}
finally {
  $client.Dispose()
}
