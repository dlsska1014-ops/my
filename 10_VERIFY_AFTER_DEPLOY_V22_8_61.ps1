...
try {
  ...
  if ($health.alive -ne $true -or $health.missing_count -ne 0 -or $health.version -ne "V22.8.61-MOBILE-SURFACE-REPAIR") {
  ...
  foreach ($assetCheck in @(
    @{ Path = "/assets/accountbook-shell-v22861.css"; Type = "text/css" },
    @{ Path = "/assets/accountbook-nav-v22861.js"; Type = "javascript" },
  ...
  Write-Host "PASS: V22.8.61 mobile surface repair"
  Write-Host "Version:" $health.version
  Write-Host "Checked RPCs:" $ready.checked_rpcs
  Write-Host "Next: on a 390px device verify the docked top-bar action button, full-menu touch targets, the bottom quick-input icon, quick-input chips and sheet scrolling, and the challenge date fields."
}
finally {
  $client.Dispose()
}
